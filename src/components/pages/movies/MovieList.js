...
export class MovieList extends Component {
  ...
  getMovies = () => {
    ...
    if (this.props.mode === "") {
      ...
      axios
        .get(`http://127.0.0.1:8000/api/movies-${this.props.mode}/`)
        .then((res) => {
          this.setState({ ...this.state, movies: res.data });
        });
    }
        ...
        <div className="container-fluid p-3">
          ...
          {this.state.movies.map((movie, index) => (
            <MovieItem
              ...
              index={index}
              launchMovie={this.launchMovie}
              takeOutMovie={this.takeOutMovie}
            />
          ))}
        </div>
      </div>
  ...
  launchMovie = (movie) => {
    ...
    swal({
      ...
      if (willDelete) {
        axios
          .put(`http://127.0.0.1:8000/api/movies/${movie.movie_id}/`, data)
          .then((res) => {
            this.getMovies(); // Se actualiza la informacion mostrada
            swal("Exitoso", "¡Se ha estrenado la película!", "info", {
              dangerMode: true,
            });
          });
      } else {
    ...
    swal({
      ...
      if (willDelete) {
        axios
          ...
          .then((res) => {
            this.getMovies(); // Se actualiza la informacion mostrada
            swal("Exitoso", "¡Se ha sacado la película del aire!", "info", {
              dangerMode: true,
            });
          });
      } else {
